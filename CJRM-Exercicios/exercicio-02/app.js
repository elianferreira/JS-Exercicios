...
let randomWordLength = randomWord.length - 1;
...
const bookMessage = '\'O Conto da Ai\' é um dos livros mais vendidos da década.';
...
let newName =
  name.slice(0, 6).replace('w', 'W') + ' ' + name.slice(7).replace('w', 'W');
console.log(newName);
...
// let newName = `${name.slice(0, 6).replace('w', 'W')} ${name
//   .slice(7, 13)
//   .replace('w', 'W')}`;
// console.log(newName);
