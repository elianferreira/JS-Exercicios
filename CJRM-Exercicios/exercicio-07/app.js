...
for (let i = 0; i < randomNumbers.length; i++) {
  acum += randomNumbers[i];
  if (acum > 400) {
    console.log(`A soma ultrapassou 400. Até aqui, o valor atual é ${acum}.`);
    break;
  }
}
...
for (let i = 0; i < randomValues.length; i++) {
  switch (typeof randomValues[i]) {
    case 'string':
      acumString += 1;
      arrayOfString.push(randomValues[i]);
      break;
    case 'boolean':
      arrayOfBoolean.push(randomValues[i]);
      console.log(arrayOfBoolean);
      break;
  }

  if (acumString == 4) {
    console.log(`As primeiras 4 strings são: ${arrayOfString.join(', ')}`);
    console.log(
      `Até que as primeiras 4 strings fossem iteradas, ${arrayOfBoolean.length} booleans foram iterados`
    );
    break;
  }
}
...
switch (drinkType) {
  ...
  case 'refrigerante':
    ...
    break;
  case 'suco':
    choseDrink = 'Bebida produzida do líquido extraído de frutos.';
    break;
  default:
    choseDrink = 'Bebida desconhecida.';
    break;
}

console.log(choseDrink);
